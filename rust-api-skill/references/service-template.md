...
impl UserService {
    ...
    pub async fn create(
        ...
        // 业务校验：邮箱唯一性
        if let Some(_) = UserRepo::find_by_email(&state.db, &req.email).await? {
        ...
        let model = UserRepo::create(&state.db, active_model).await?;
        Ok(model.into())
    }
    ...
    pub async fn list(
        ...
        let (items, total) = UserRepo::find_paginated(&state.db, page, page_size, keyword).await?;
        Ok(UserListResp {
    ...
    pub async fn update(
        ...
        let existing = UserRepo::find_by_id(&state.db, id)
            .await?
        ...
        let model = UserRepo::update(&state.db, active_model).await?;
        Ok(model.into())
    }

    /// 删除用户
    pub async fn delete(state: &AppState, id: Uuid) -> Result<(), AppError> {
        UserRepo::find_by_id(&state.db, id)
            .await?
            .ok_or(AppError::NotFound("用户不存在".to_string()))?;
        UserRepo::delete_by_id(&state.db, id).await
    }
}
```
